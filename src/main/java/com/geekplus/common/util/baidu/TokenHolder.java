...
public class TokenHolder {
     ...
     * @throws BaseException http接口返回不是 200, access_token未获取
     */
    public void refresh() throws IOException, BaseException {
     ...
     */
    private void parseJson(String result) throws BaseException {
        JSONObject json = JSONObject.parseObject(result);

        if (!json.containsKey("access_token")) {
            // 返回没有access_token字段
            throw new BaseException("access_token not obtained, " + result);
        }
        if (!json.containsKey("scope")) {
            // 返回没有scope字段
            throw new BaseException("scope not obtained, " + result);
        }
        if (!json.getString("scope").contains(scope)) {
            throw new BaseException("scope not exist, " + scope + "," + result);
        }
